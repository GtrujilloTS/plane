...
class S3Storage(S3Boto3Storage):
    ...
    def __init__(self, request=None):
        ...
        if os.environ.get("USE_MINIO") == "1":
            ...
            # When MINIO_EXTERNAL_URL is set (local dev), always use the internal
            # Docker endpoint so presigned URLs consistently use the internal
            # hostname, which generate_presigned_post then swaps for the external one.
            if os.environ.get("MINIO_EXTERNAL_URL"):
    ...
    def generate_presigned_post(self, object_name, file_type, file_size, expiration=None):
        ...
        # In local dev, MinIO generates URLs with the internal Docker hostname
        # (plane-minio:9000) which the browser can't resolve. Replace it with
        # the externally accessible URL. The signature stays valid because for
        # presigned POST, it covers the policy fields, not the URL host.
        external_url = os.environ.get("MINIO_EXTERNAL_URL")
        if external_url and response and self.aws_s3_endpoint_url:
            response["url"] = response["url"].replace(self.aws_s3_endpoint_url, external_url)
